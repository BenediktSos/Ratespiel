		...
		while (true) {
			...
			if (!autosolve) {
				...
				if (neededTime > (Math.sqrt((maxBorder - minBorder) * 100) + 5)) {
					...
					frame.appendHistory("Ich bin nicht ent�uscht,...");
					try {
						Thread.sleep(2000);
					} catch (InterruptedException e) {
						e.printStackTrace();
					...
					frame.appendHistory("Ich h�tte mehr von dir erwartet.");
				}
			} else {
				frame.appendHistory(" um die L�sung zu finden.");
			...
			if (askYesNo(frame, "Spiel beenden?(y/N)")) {
				...
				if (askYesNo(frame, "Zahlengrenzen �ndern?(y/N)")) {
